-- Anime Expeditions dashboard: denormalize summon pity onto the light
-- `accounts` table (same treatment as currencies/progress/stats) so the main
-- dashboard list can show it without a per-row account_details fetch.

alter table public.accounts add column if not exists pity jsonb not null default '{}'::jsonb;
...
begin
    ...
    v_equip := coalesce(p->'Equipment', '[]'::jsonb);

    insert into public.accounts as a
        (user_id, owner_user_id, username, display_name, level, exp, currencies, progress,
         stats, pity, unit_count, item_count, in_match, payload_hash, last_seen, updated_at)
    values
        (v_user_id, v_owner,
    ...
    insert into public.account_details as d (user_id, units, inventory, equipment, raw, updated_at)
    values (v_user_id, v_units, v_inv, v_equip, p - 'CapturedAt', now())
    on conflict (user_id) do update set
